    ...
    if (isset($_POST['add_music'])) {
        if (!empty($_FILES)) {
            ...
            if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                $sql = "INSERT INTO `fashion`(`description`, `image`, `tribe`) VALUES ('$description', '$targetFile', '$tribe')";
                $results = $conn->query($sql);
                if ($results) {
                    $username =  $_SESSION['user']['username'];
                    $transaction_id = "#" . date('Ym') . time();
                    $sql = "INSERT INTO `logs`(`transaction_id`, `transaction_type`, `user`) VALUES ('$transaction_id', 'Added fashion and design food into the system', '$username')";
                    $conn->query($sql);
                }
            }
        }
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0 font-size-18">Fashion and Design</h4>
                            </div>
                        </div>
                    </div>
                    <!-- end page title -->
                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <span>
                                                <button type="button" class="js-swal-confirm btn btn-success mb-3" data-bs-toggle="modal" data-bs-target="#addtribe">
                                                    <i class="fa fa-plus text-white me-1"></i> Record
                                                </button>
                                            </span>
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <table id="datatable" class="table table-bordered dt-responsive">
                                            ...
                                            <tbody>
                                                ...
                                                $sql = "SELECT * FROM `fashion` JOIN tribes ON fashion.tribe = tribes.tribe_id ORDER BY fashion.id DESC";
                                                $results = $conn->query($sql);
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="exampleModalLabel">Fashion and Design</h1>
