    ...
    if (isset($_POST['add_music'])) {
        if (!empty($_FILES)) {
            ...
            if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                $sql = "INSERT INTO `music`(`music_title`, `music_file`, `tribe_id`, `music_description`) 
                            VALUES ('$title', '$targetFile', '$tribe', '$description')";
                $results = $conn->query($sql);
                if ($results) {
                    $username =  $_SESSION['user']['username'];
                    $transaction_id = "#" . date('Ym') . time();
                    $sql = "INSERT INTO `logs`(`transaction_id`, `transaction_type`, `user`) VALUES ('$transaction_id', 'Added new music into the system', '$username')";
                    $conn->query($sql);
                }
            }
        }
                                    ...
                                    <div class="card-body">
                                        <table id="datatable" class="table table-bordered dt-responsive">
                                            ...
                                            <tbody>
                                                ...
                                                $sql = "SELECT * FROM `music` JOIN tribes ON music.tribe_id = tribes.tribe_id ORDER BY music_id DESC";
                                                $results = $conn->query($sql);
                                                while ($music = $results->fetch_assoc()) {
                                                ?>
                                                    <tr>
                                                        <td><?php echo $music['music_title'] ?></td>
                                                        <td><?php echo $music['name']?></td>
                                                        <td>
                                                            <?php

                                                            echo time_ago($music['music_added'])
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="exampleModalLabel">Add Music</h1>
                            ...
                            <div class="modal-body">
                                <form class="js-validation-signin" action="" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-4">
                                        <div class="input-group input-group-lg">
                                            <textarea class="form-control" name="description" placeholder="Music Description"></textarea>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-success" name="add_music">Save Music</button>
                            </div>
                            </form>
                        </div>
                    </div>
